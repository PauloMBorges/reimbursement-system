...
const globalForPrisma = globalThis as unknown as { // Type assertion necessária para acessar 'prisma' em 'globalThis' (que é do tipo 'any')
    ...
    new PrismaClient({
        log:
            env.NODE_ENV === "development"
                ? ["query", "error", "warn"]
                : ["error"], // Em prod só loga erros (economiza recursos)
    });
